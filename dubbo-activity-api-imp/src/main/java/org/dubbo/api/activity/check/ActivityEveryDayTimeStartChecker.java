...
public class ActivityEveryDayTimeStartChecker extends Checker {

    Logger logger = Logger.getLogger(ActivityTimeStartChecker.class);

    public ActivityEveryDayTimeStartChecker(Checker nextChecker, boolean isOpen){
    ...
    public ActivityCheckResult doCheck(ActivityCheckJson activityCheckJson) {
        ...
        long time = 0l;
        try {
            time = simpleDateFormat.parse(simpleDateFormat2.format(System.currentTimeMillis()) + activityCheckJson.getEveryDayStartTime()).getTime();
